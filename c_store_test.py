...
def store_image(file, StUID, SeUID, pid, acc, namespell, name, studydate):
	# 1.读取影像
	ds = dcmread(file)
	# 2.修改影像值
	...
	ds.PatientName = namespell
	ds.OtherPatientNames = name
	# ds.StudyDate = studydate.split(' ')[0]
	ds.StudyDate = convert_date(studydate)[0]
	ds.StudyTime = convert_date(studydate)[1]
